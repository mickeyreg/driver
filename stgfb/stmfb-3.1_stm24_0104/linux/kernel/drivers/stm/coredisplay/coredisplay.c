...
{
  int res;
  platform_data->class_device = device_create(stmcore_class,
                                              NULL, MKDEV(0,0), platform_data,
                                              "display%d",i);
...
int __init stmcore_display_init(void)
{
  ...
  for(i=0;i<nr_platform_devices;i++)
  {
    ...
    platform_data[i].display_runtime->hotplug_poll_pio = -1;
