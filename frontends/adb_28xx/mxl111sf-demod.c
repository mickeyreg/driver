...
/* debug */
static int mxl111sf_demod_debug=0;
					...
					int *sync_lock)
{
	...
	*sync_lock = (val & SYNC_LOCK_MASK) >> 4;
	//printk("mxl1x1sf_demod_get_sync_lock_status %d\n",ret);
	return ret;
fail:
	printk("mxl1x1sf_demod_get_sync_lock_status fail %d\n",ret);
	return ret;
}
				      ...
				      int *rs_lock)
{
	...
	*rs_lock = (val & RS_LOCK_DET_MASK) >> 3;
	//printk("mxl1x1sf_demod_get_rs_lock_status %d\n",ret);
	return ret;
fail:
	printk("mxl1x1sf_demod_get_rs_lock_status fail %d\n",ret);
	return ret;
}
				       ...
				       int *tps_lock)
{
	...
	*tps_lock = (val & V6_PARAM_TPS_LOCK_MASK) >> 6;
	//printk("mxl1x1sf_demod_get_tps_lock_status %d\n",ret);
	return ret;
fail:
	printk("mxl1x1sf_demod_get_tps_lock_status fail %d\n",ret);
	return ret;
}
				       ...
				       int *fec_lock)
{
	...
	*fec_lock = (val & IRQ_MASK_FEC_LOCK) >> 4;
	//printk("mxl1x1sf_demod_get_fec_lock_status %d\n",ret);
	return ret;
fail:
	printk("mxl1x1sf_demod_get_fec_lock_status fail %d\n",ret);
	return ret;
}
				      ...
				      int *cp_lock)
{
	...
	*cp_lock = (val & V6_CP_LOCK_DET_MASK) >> 2;
	printk("mxl1x1sf_demod_get_cp_lock_status %d\n",ret);
	return ret;
fail:
	return ret;
	printk("mxl1x1sf_demod_get_cp_lock_status fail %d\n",ret);
}
*/
 ...
 * return zero for now. */
//#define CALCULATE_BER(avg_errors, count) ((u32)(avg_errors * 4)/(count*64*188*8))
//#define CALCULATE_SNR(data) ((u32)((10 * (u32)data / 64) - 2.5))
//#else
				   ...
				   u16 *snr)
{
	...
	*snr = 0;
	...
	//printk("snr val1:%d val2:%d\n",val1,val2);

//	*snr = CALCULATE_SNR(val1 | ((val2 & 0x03) << 8));
	*snr = (val1 | ((val2 & 0x03) << 8))*100;
fail:
	return ret;
}
...
static int mxl111sf_demod_read_snr(struct dvb_frontend *fe, u16 *snr)
{
	...
	//*snr /= 10; /* 0.1 dB */
	printk("mxl101sf_read_snr:%d\n",*snr);
fail:
	return ret;
}

static int mxl111sf_get_property(struct dvb_frontend *fe, struct dtv_property* tvp)
{
...
{
	struct mxl111sf_demod_state *state = fe->demodulator_priv;
	int ret, locked, cr_lock, sync_lock, fec_lock;//, cp_lock;
	...
	*status = 0;
	...
	if (mxl_fail(ret))
		goto fail;
//	ret = mxl1x1sf_demod_get_cp_lock_status(state, &cp_lock);
//	if (mxl_fail(ret))
//		goto fail;

	if (locked)
	{*status |= FE_HAS_SIGNAL;}//printk("mxl111sf_demod_read_status FE_HAS_SIGNAL\n");}
	if (cr_lock)
	{*status |= FE_HAS_CARRIER;}//printk("mxl111sf_demod_read_status FE_HAS_CARRIER\n");}
	if (sync_lock)
	{*status |= FE_HAS_SYNC;}//printk("mxl111sf_demod_read_status FE_HAS_SYNC\n");}
	if (fec_lock) // false positives?
	{*status |= FE_HAS_VITERBI;}//printk("mxl111sf_demod_read_status FE_HAS_VITERBI\n");}
	if ((locked) && (cr_lock) && (sync_lock))
	{*status |= FE_HAS_LOCK;}//printk("mxl111sf_demod_read_status FE_HAS_LOCK\n");}

	//printk("mxl111sf_demod_read_status %d\n",ret);
	return 0;
fail:
	printk("mxl111sf_demod_read_status fail %d\n",ret);
	return ret;
}
					       ...
					       u16 *signal_strength)
{
	...
	*signal_strength = *signal_strength * 5;
	...
	printk("mxl101sf_read_signal_strength:%d\n",*signal_strength);
	return 0;
}
...
static struct dvb_frontend_ops mxl111sf_demod_ops = {
	...
	.get_property         = mxl111sf_get_property,
