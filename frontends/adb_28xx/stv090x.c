...
#if (LINUX_VERSION_CODE == KERNEL_VERSION(2,6,23)) && (defined(FORTIS_HDBOX) || defined(HS7810A) || defined(HS7110) || defined(WHITEBOX))
...
int pwm_diseqc_init(void)
{
	...
	//debug_diseqc_pwm("reg div = 0x%x\n", reg);

	writel(reg,PWM_CTRL);//generowanie przerwania co 500us
...
static int stv090x_set_tone(struct dvb_frontend *fe, fe_sec_tone_mode_t tone)
{
	...
	case SEC_TONE_ON:
		#if defined(ADB2850)
		if(mp8125_extm!=NULL) stpio_set_pin (mp8125_extm, 1);
		#endif
	...
	case SEC_TONE_OFF:
		#if defined(ADB2850)
		if(mp8125_extm!=NULL) stpio_set_pin (mp8125_extm, 0);
		#endif
		STV090x_SETFIELD_Px(reg, DISTX_MODE_FIELD, 0);
		STV090x_SETFIELD_Px(reg, DISEQC_RESET_FIELD, 1);
		if (STV090x_WRITE_DEMOD(state, DISTXCTL, reg) < 0)
...
static int stv090x_send_diseqc_msg(struct dvb_frontend *fe, struct dvb_diseqc_master_cmd *cmd)
{
	...
	if(box_type==ADB2849) pwm_diseqc1_send_msg (fe,cmd);
	else
	{
...
static int stv090x_send_diseqc_burst(struct dvb_frontend *fe, fe_sec_mini_cmd_t burst)
{
	...
	if(box_type==ADB2849) pwm_send_diseqc1_burst(fe,burst);
	else
	{
...
static int stv090x_init(struct dvb_frontend *fe)
{
	...
	if(box_type==ADB2849)
		...
		if (mp8125_en==NULL) {printk("!!!!!! FAIL : request pin 6 0\n");}
		else
		stpio_set_pin (mp8125_en, 0);

		mp8125_13_18 = stpio_request_pin(6, 5, "mp8125_13_18", STPIO_OUT);
		if (mp8125_13_18==NULL) {printk("!!!!!! FAIL : request pin 6 5\n");}
		else
		stpio_set_pin (mp8125_13_18, 0);
		}
	//adb2850 pio7/3 only tone
	//adb2849 pio7/3 tone + diseqc
	mp8125_extm = stpio_request_pin(7, 3, "mp8125_extm", STPIO_OUT);
	if (mp8125_extm==NULL) {printk("!!!!!! FAIL : request pin 7 3\n");}
	else
		{
		stpio_set_pin (mp8125_extm, 0);
		if(box_type==ADB2849) pwm_diseqc_init();
		}
	}
...
#if defined(ADB2850) || defined(HS7810A) || defined(HS7110) || defined(WHITEBOX) || defined(SPARK)
...
#elif defined(HS7810A) || defined(HS7110) || defined(WHITEBOX) || defined(SPARK)
...
#elif defined(ADB2850) // TODO move to lnb driver
...
//     if(_12v_isON == 0)
//	   {
		if(box_type==ADB2849)stpio_set_pin (mp8125_en, 0);
		else
	   	{
		lnbh23_v=0;
